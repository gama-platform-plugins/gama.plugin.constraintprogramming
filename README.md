...
The operators that only a constraint engine can honour ask for it by name, with `requireChoco`, which returns the engine and checks the problem has one in the same step. There is no way to reach a Choco solver without saying which operator is asking and what the alternative is, which is what the error message then says.
