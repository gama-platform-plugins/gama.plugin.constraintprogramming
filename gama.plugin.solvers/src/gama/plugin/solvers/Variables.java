...
public class Variables {
	...
	public static GamaVariable intVarIn(final IScope scope, final GamaProblem problem, final String name,
		...
		if (problem == null) throw GamaRuntimeException.error("Cannot declare the variable " + name + " in a nil problem", scope);
		final int[] domain = SolverUtils.ints(scope, values);
		if (domain.length == 0)
	...
	public static GamaVariable setVar(final IScope scope, final GamaProblem problem, final String name,
		...
		if (problem == null) throw GamaRuntimeException.error("Cannot declare the variable " + name + " in a nil problem", scope);
		SolverUtils.requireConstraintEngine(scope, problem, "set_var", "A linear engine has no notion of a set variable.");
		return problem
				.register(problem.getModel().setVar(name, SolverUtils.ints(scope, mandatory), SolverUtils.ints(scope, possible)));
	}
	...
	public static GamaVariable minVar(final IScope scope, final IList<GamaVariable> vars) throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, vars);
		SolverUtils.requireConstraintEngine(scope, p, "min_var", null);
		return p.register(p.getModel().min(p.newName("min"), SolverUtils.intVars(scope, vars)));
	}
	...
	public static GamaVariable maxVar(final IScope scope, final IList<GamaVariable> vars) throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, vars);
		SolverUtils.requireConstraintEngine(scope, p, "max_var", null);
		return p.register(p.getModel().max(p.newName("max"), SolverUtils.intVars(scope, vars)));
	}
	...
	public static GamaVariable countVar(final IScope scope, final IList<GamaVariable> vars, final int value)
			throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, vars);
		SolverUtils.requireConstraintEngine(scope, p, "count_var", null);
		return p.register(p.getModel().count(p.newName("count"), value, SolverUtils.intVars(scope, vars)));
	}
	...
	public static GamaVariable argMinVar(final IScope scope, final IList<GamaVariable> vars) throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, vars);
		SolverUtils.requireConstraintEngine(scope, p, "arg_min_var", null);
		return p.register(p.getModel().argmin(p.newName("argmin"), SolverUtils.intVars(scope, vars)));
	}
	...
	public static GamaVariable argMaxVar(final IScope scope, final IList<GamaVariable> vars) throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, vars);
		SolverUtils.requireConstraintEngine(scope, p, "arg_max_var", null);
		return p.register(p.getModel().argmax(p.newName("argmax"), SolverUtils.intVars(scope, vars)));
	}
	...
	public static GamaVariable elementVar(final IScope scope, final IList<Integer> table, final GamaVariable index)
			throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, index);
		SolverUtils.requireConstraintEngine(scope, p, "element_var", null);
		return p.register(
				p.getModel().element(p.newName("element"), SolverUtils.ints(scope, table), index.asIntVar(scope), 0));
	}
	...
	public static GamaVariable modVar(final IScope scope, final GamaVariable var, final int divisor)
		...
		SolverUtils.requireConstraintEngine(scope, p, "mod_var", "Write it as an expression instead, as in x mod k.");
	...
	public static GamaVariable absVar(final IScope scope, final GamaVariable var) throws GamaRuntimeException {
		final GamaProblem p = SolverUtils.problemOf(scope, var);
		SolverUtils.requireConstraintEngine(scope, p, "abs_var", null);
